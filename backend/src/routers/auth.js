...
router.post("/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id, isVerified, info, email }, config.jwtSecret, {
      expiresIn: "2d",
    });
...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (existingUser) return res.status(409).json({ message: "Email already to be used" });
...
router.put("/:userId", async (req, res) => {
  try {
    ...
    jwt.verify(token, config.jwtSecret, async (err, decode) => {
      ...
      const { id } = decode;
      if (id !== userId)
        return res.status(401).json({ message: "Not allowed to update that user's data" });
      ...
      const newToken = jwt.sign({ id, isVerified, info, email }, config.jwtSecret, {
        expiresIn: "2d",
      });
